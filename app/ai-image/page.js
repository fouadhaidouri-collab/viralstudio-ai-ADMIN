            ...
            {options.map((opt) => {
              ...
              return (
                ...
                >
                  ...
                  {selected && <span className="material-symbols-outlined text-xs ml-1 text-primary" style={{ fontVariationSettings: "'FILL' 1" }}>check</span>}
                </button>
              );
            })}
          </div>
  ...
  return (
    <div className="h-screen overflow-hidden no-x-scroll">
      ...
      <div className="fixed inset-0 overflow-hidden z-0">
        <video ref={bgVideoRef} src={TEMPLATE_VIDEOS[bgVideoIdx]} muted autoPlay playsInline className="w-full h-full object-fill opacity-60" />
                  ...
                  {selectedImageIndex !== null ? (
                    <div className="flex flex-col gap-3">
                      <button onClick={() => setSelectedImageIndex(null)} className="flex items-center gap-1.5 text-xs text-on-surface-variant hover:text-white transition-colors">
                        <span className="material-symbols-outlined text-sm">arrow_back</span> Back to all
                      </button>
